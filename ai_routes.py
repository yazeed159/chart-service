...
def backtest_ai():
    ...
    system_parts = [
        ...
        "is chosen, and you should skip the ones that don't apply to whatever entry_mode/stop_mode/",
        "position_sizing_mode the trader picked. But do NOT mark this ready just because the handful of",
        "structural fields (start/end/entry_mode/stop_mode/position_size) are resolved -- a config with",
        "no scan filters, a default position size the trader never actually chose, and no exit refinement",
        "isn't a solid backtest, it's a placeholder. Before setting status to \"ready\", you must have",
        "explicitly raised each of these groups with the trader at least once (a quick answer or an",
        "explicit \"just use a sensible default\" counts as resolved -- silently picking a number without",
        "ever asking does NOT):",
        "  1. Universe/scan filters -- top_n, min_price, max_price, min_dollar_volume, min_gap_pct",
        "     (these define which setups the backtest even looks at; skipping them means testing an",
        "     undefined universe).",
        "  2. Capital & sizing -- starting_capital, position_sizing_mode, and whichever of",
        "     position_size / position_size_pct / risk_pct_of_capital that mode implies.",
        "  3. Session timing -- session_start, flatten_time.",
        "  4. Entry mechanics -- entry_mode plus its mode-specific fields.",
        "  5. Stop mechanics -- stop_mode plus its mode-specific fields.",
        "  6. Exit refinement -- at minimum ask about target_r and whether they want a breakeven-arm,",
        "     time stop, or giveback-off-peak exit rather than riding the stop the whole way; these are",
        "     what separate a real strategy from a bare entry/stop pair.",
        "  7. Trade frequency -- allow_reentry / max_trades_per_day (only ask about",
        "     reentry_cooldown_minutes if allow_reentry ends up true).",
        "  8. Position building (OPTIONAL -- only raise this if the trader brings up adding to winners,",
        "     scaling in/out, pyramiding, 'starting small', or a Ross-Cameron-style building-a-position",
        "     approach; don't ask about it for a plain single-fill strategy): scale_in_enabled and, if on,",
        "     scale_in_initial_size_pct / scale_in_add_size_pct / scale_in_max_adds / scale_in_hold_bars /",
        "     scale_in_min_gain_cents -- adds trigger on 'new high, holds for scale_in_hold_bars bars,",
        "     breaks again', never on a trade that isn't already working. Separately, trail_protect_enabled",
        "     and its trail_protect_ladder (list of [peak_r_multiple, protect_fraction_0_to_1] pairs, e.g.",
        "     [[0.5,0.3],[1,0.5],[2,0.65],[3,0.8]] means 'lock in 30% of the peak gain once up half a risk-",
        "     unit, 50% by 1R, 65% by 2R, 80% by 3R+' -- the default deliberately starts below 1R so a",
        "     trade that never quite reaches 1R before reversing still has something protected instead of",
        "     being able to round-trip all the way back to the original stop) -- a stop that ratchets up to",
        "     protect a GROWING share of the move instead of one fixed",
        "     giveback the whole time. These two are independent of each other and of target_r/giveback_*/",
        "     stall_exit -- a trader can combine any of them.",
        "Ask about these in whatever order flows naturally from the conversation -- still one question at",
        "a time, never a checklist dump -- but track which groups haven't been raised yet and keep going",
        "until each one has, even if the trader's answer for a group is just \"default is fine\". Only then",
        "does everything left over (label, include_commissions, notes) get to default silently.",
        "",
        ...
        "cover, selling part of the position at a fixed profit target (partial exits -- the engine is",
        "all-in/all-out on every exit; group 8's trail_protect ladder is the closest supported analogue,",
        "since it locks in more of the move as it runs instead of taking a partial off at a fixed price),",
        "discretionary judgment calls). When the trader describes something like that:",
